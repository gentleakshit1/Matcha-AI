  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans text-slate-900">
      ...
      <header className="bg-white/80 backdrop-blur-md border-b border-slate-200 h-16 flex items-center justify-between px-8 sticky top-0 z-50 shadow-sm">
        <div className="flex items-center gap-4 cursor-pointer" onClick={() => navigate('/')}>
          <h1 className="text-xl font-extrabold tracking-tight flex items-center gap-2">
            <div className="w-8 h-8 brand-gradient-bg rounded-lg flex items-center justify-center text-white shadow-soft">🍵</div>
        ...
        <div className="flex items-center gap-4">
          {!isSignedIn && (
            <button
              ...
              className="btn-dark btn-sm rounded-full"
            >
              Sign In
            </button>
          )}
      ...
      <div className="bg-slate-900 text-white py-20 px-8 text-center relative overflow-hidden">
        ...
        <p className="label-eyebrow text-green-400 mb-4 relative">Join the Team</p>
        <h2 className="text-5xl font-extrabold mb-6 tracking-tight relative">Open Positions</h2>
        ...
        <div className="flex flex-col gap-5">
          {availableJobs.length === 0 ? (
            ...
            availableJobs.map((job, index) => {
              ...
              return (
                <div key={job.id} className="bg-white rounded-2xl border border-slate-200 hover:border-green-300 hover:shadow-soft-lg shadow-soft transition-all duration-200 overflow-hidden group">
                  <div className="p-6 cursor-pointer" onClick={(e) => toggleJobExpand(job.id, e)}>
                    <div className="flex justify-between items-start mb-3">
                      <h4 className="text-xl font-extrabold text-slate-900 group-hover:text-green-700 transition-colors flex items-center gap-2">
                        <span className="text-sm bg-slate-100 text-slate-500 px-2.5 py-1 rounded-md border border-slate-200 font-bold">Pipeline #{visualId}</span>
                        {job.title}
                      </h4>
      ...
      {/* Edit JD Modal */}
      {editingJob && (
        <div className="modal-backdrop" role="dialog" aria-modal="true" aria-labelledby="edit-jd-title">
          <div className="modal-panel max-w-2xl p-8">
            <div className="flex justify-between items-center mb-6">
              <h2 id="edit-jd-title" className="text-xl font-extrabold text-slate-900">Edit Job Description</h2>
              <button onClick={() => setEditingJob(null)} className="p-2 text-slate-400 hover:text-slate-700 hover:bg-slate-100 rounded-full transition-colors" aria-label="Close">
                <X size={20} />
              </button>
            </div>
            ...
            <form onSubmit={handleEditSave} className="flex flex-col gap-4">
              <div>
                <label htmlFor="edit-jd-title-input" className="field-label">Job Title</label>
                <input
                  id="edit-jd-title-input"
                  required
              ...
              <div>
                <label htmlFor="edit-jd-text" className="field-label">Raw Job Description Text</label>
                <textarea
                  id="edit-jd-text"
                  required
              ...
              <div className="flex gap-4 w-full mt-4">
                ...
                >
                  {isSaving ? (
                    <><span className="animate-spin rounded-full h-4 w-4 border-2 border-white/40 border-t-white"></span> Updating AI Agent...</>
                  ) : "Save & Sync with AI"}
                </button>
              </div>
            </form>
